...
import {
	...
	SummarySection,
	BackButton,
} from '@/pages/ticketingpage/styles/commonStyles';
...
const Step2 = ({
	...
	const deliveryOptions = [{ value: 'venue', label: '현장 수령' }];

	const paymentOptions = [{ value: 'pay', label: '카카오페이' }];

	// 할인(티켓) 선택 핸들러
	const handleDiscountChange = (ticketId) => {
		const selectedOption = discountOptions.find(
			(option) => option.value === ticketId,
		);
		if (selectedOption) {
	...
	if (
		loading &&
		(currentContent === 'payment' || currentContent === 'options')
	) {
	...
	if (error) {
		return (
			<div style={{ textAlign: 'center', padding: '50px', color: '#F67676' }}>
				<p>{error}</p>
				<ActionButton isActive={true} onClick={() => window.location.reload()}>
					다시 시도
				</ActionButton>
			</div>
		);
	...
	return (
		<>
			{!isPC && <PosterInfo eventInfo={eventInfo} />}
			{isPC && (
				<>
			...
			<PcLayout>
				...
				{(showPaymentOnly ||
					(showAllOptions && discountType && deliveryType)) && (
					<FormSection2>
						{!isPC && <SectionTitle>결제</SectionTitle>}
						<SelectionSection>
							{!isPC && <Label>결제 수단</Label>}
							{isPC && (
								<label
									style={{
										fontSize: '20px',
				...
				<SummarySection>
					...
					<SummaryRow>
						...
						<span>
							{deliveryType
								? `${payment.deliveryFee.toLocaleString()}원 (현장수령)`
								: ''}
						</span>
					</SummaryRow>
					<SummaryRow className="total">
						...
						{!isPC && (
							<span>
								{(payment.totalPrice * (people || 1)).toLocaleString()}원
							</span>
						)}
						{isPC && (
							<span style={{ fontSize: '24px', bottom: '4px' }}>
								{(payment.totalPrice * (people || 1)).toLocaleString()}원
							</span>
						)}
					</SummaryRow>
